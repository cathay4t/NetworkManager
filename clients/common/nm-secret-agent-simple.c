...
static void
_auth_dialog_exited (GPid pid, gint status, gpointer user_data)
{
	...
	if (!g_key_file_load_from_data (keyfile,
	                                ...
	                                &error)) {
		g_set_error (&error, NM_SECRET_AGENT_ERROR, NM_SECRET_AGENT_ERROR_FAILED,
		             "Bad response from the auth dialog: %s\n", error->message);
		goto out;
	}
	...
	if (!title) {
		g_set_error (&error, NM_SECRET_AGENT_ERROR, NM_SECRET_AGENT_ERROR_FAILED,
		             "Missing Title in auth dialog response: %s\n", error->message);
		goto out;
	}
	...
	if (!message) {
		g_set_error (&error, NM_SECRET_AGENT_ERROR, NM_SECRET_AGENT_ERROR_FAILED,
		             "Missing Description in auth dialog response: %s\n", error->message);
		goto out;
	}
...
_add_data_item_to_string (const char *key, const char *value, gpointer user_data)
{
	...
	_add_to_string (string, "DATA_VAL", key);
	g_string_append_c (string, '\n');
}
...
_add_secret_to_string (const char *key, const char *value, gpointer user_data)
{
	...
	_add_to_string (string, "SECRET_VAL", key);
	g_string_append_c (string, '\n');
}

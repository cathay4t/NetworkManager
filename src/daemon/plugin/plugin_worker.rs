...
use super::plugin_exec::NmDaemonPlugin;
...
#[derive(Debug, Clone, PartialEq)]
pub(crate) enum NmPluginReply {
    None,
    States(Vec<NetworkState>),
}

type FromManager = (NmPluginCmd, Sender<Result<NmPluginReply, NmError>>);

#[derive(Debug)]
pub(crate) struct NmPluginWorker {
    receiver: UnboundedReceiver<FromManager>,
    plugins: HashMap<String, NmDaemonPlugin>,
}

impl TaskWorker for NmPluginWorker {
    type Cmd = NmPluginCmd;
    type Reply = NmPluginReply;
    ...
    async fn process_cmd(
        ...
        match cmd {
            NmPluginCmd::QueryNetworkState(opt) => {
                ...
                Ok(NmPluginReply::States(ret))
            }
            NmPluginCmd::ApplyNetworkState(v) => {

...
impl OvsDbJsonRpc {
    pub(crate) async fn connect(socket_path: &str) -> Result<Self, NmError> {
        Ok(Self {
            socket: UnixStream::connect(socket_path).await.map_err(|e| {
                NmError::new(ErrorKind::Bug, format!("socket error {e}"))
            })?,
        })
    }

    pub(crate) async fn send(&mut self, data: &Value) -> Result<(), NmError> {
    ...
    pub(crate) async fn recv(
        &mut self,
        transaction_id: u64,
    ) -> Result<Value, NmError> {
        let mut response: Vec<u8> = Vec::with_capacity(BUFFER_SIZE);

        let mut reply: Result<OvsDbRpcReply, NmError> = Err(NmError::new(
            ErrorKind::PluginFailure,

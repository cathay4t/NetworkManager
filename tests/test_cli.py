...
def test_cli_wait_carrier_up(veth1_down):
    ...
    assert (
        exec_cmd("nmc wait veth1 up --timeout 1".split(), check=False)[0] != 0
    )

    exec_cmd("ip link set veth1ep up".split())
    # Should get timeout on waiting veth1 up because other end is down
    assert (
        exec_cmd("nmc wait veth1 up --timeout 1".split(), check=False)[0] != 0
    )

    exec_cmd("ip link set veth1ep down".split())
    exec_cmd("ip link set veth1 up".split())
    ...
    exec_cmd("ip link set veth1ep up".split())
    exec_cmd("ip link set veth1 up".split())
    assert exec_cmd("nmc wait veth1 up --timeout 5".split())[0] == 0
